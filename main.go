...
var registry = []*Command{
	...
	{Path: []string{"adviser", "discover"}, Summary: "the existing agents you may consult: selector, availability, the runner each reported, last seen; creates and grants nothing", Surface: "hosted",
		Args:     []Arg{{Name: "name", Required: false}},
		Flags:    []Flag{{Name: "project", Kind: flagString, Value: "ID", Summary: "one project you may read instead of your own account"}},
		Needs:    "agent.workspace",
		Effects:  "reads the agent registry; nothing is woken, called, created or connected. A name several agents share is refused with the candidates named; connecting is ks adviser connect",
		Examples: []string{"ks adviser discover", "ks adviser discover reviewer/main", "ks adviser discover --project proj_0123"},
		Nothing:  "Nothing was read.", Run: hosted(hostedAdviserDiscover)},
	{Path: []string{"advice"}, Group: true, Summary: "the advice an instruction received: each adviser's own words, attributed and unverified, and every missing opinion named; never a consensus, vote or score", Surface: "hosted"},
	{Path: []string{"advice", "list"}, Summary: "every consultation one instruction raised: who was asked, who replied, who is pending or unavailable (named), whether the set is complete, current or history", Surface: "hosted",
		Args:  []Arg{{Name: "instruction", Required: true}},
		Needs: "advisers.advice", Fallback: "agent.workspace",
		Effects:  "reads the instruction's consultations; nothing is asked, answered or decided. Each reply is shown as its own adviser's words, marked unverified; no consensus, vote, score or percentage is computed",
		Examples: []string{"ks advice list tsk_0123abcd"},
		Nothing:  "Nothing was read.", Run: hosted(hostedAdviceList)},
	{Path: []string{"advice", "show"}, Summary: "one consultation in full: who asked whom, when it was asked, taken up and answered, the context it was asked with, and the adviser's own words", Surface: "hosted",
		Args:  []Arg{{Name: "consultation", Required: true}},
		Needs: "advisers.advice", Fallback: "agent.workspace",
		Effects:  "reads one consultation; nothing changes. The reply is the adviser's own words, attributed and unverified; advice on an ended instruction is shown as its history",
		Examples: []string{"ks advice show csl_0123abcd"},
		Nothing:  "Nothing was read.", Run: hosted(hostedAdviceShow)},
	...
	{Path: []string{"cruise", "approve"}, Summary: "lock the draft, binding the upload selection, the check's inputs and one provider key per ladder provider into the digest", Surface: "hosted",
		Flags:    []Flag{{Name: "key", Kind: flagList, Value: "PROVIDER=KEY", Summary: "the key to bind for a provider with several enabled keys (repeatable)"}},
		Effects:  "reads your provider keys (one request), writes the binding into .keepstate/cruise.json -- the upload selection digest, the check and its inputs digest, and the provider key and key version per provider the ladder uses -- then locks its digest in .keepstate/cruise.lock; a run refuses, sending nothing, if any of them changed",
	...
	{Path: []string{"cruise", "status"}, Summary: "the job (or the newest): state, rung, each attempt, save points, spend, verdict, artifact", Surface: "hosted",
		...
		Flags:    []Flag{{Name: "watch", Kind: flagBool, Summary: "follow the job live: its plain state, the attempt in flight, a pending verifier result, the last save point, spend kept apart (unavailable is never $0) and the next actions, re-read as events arrive at the pace the service names; Ctrl-C ends only the watch"}},
		Examples: []string{"ks cruise status", "ks cruise status <job>", "ks cruise status <job> --watch"},
	...
	{Path: []string{"cruise", "review"}, Summary: "a job in review: why it stopped, every attempt with its exact check result and cost, what remains of the ceiling, and each option with its effect", Surface: "hosted",
		Effects:  "reads the job; decides nothing. A figure the service does not have reads unavailable, never $0; a cancelled job's cleanup is shown apart from its costs",
		Examples: []string{"ks cruise review <job>"},
		Args:     []Arg{{Name: "job", Required: true}},
		Nothing:  "Nothing was read.", Run: cruiseReview},
	{Path: []string{"cruise", "cancel"}, Summary: "cancel a job", Surface: "hosted",
		Effects:  "shows the cancel's effect, then stops the job; the service records it as your decision. Its sessions are released through a tracked cleanup, reported apart from the costs, which stand",
	...
	{Path: []string{"cruise", "resume"}, Summary: "resume a job from review, on the same or a wider ladder", Surface: "hosted",
		Effects:  "shows the option's effect and what it may spend (what remains of the unchanged ceiling, or that this is not known), then starts new attempts from the parked parent, billed like the first ones; the service records it as your decision on a new approval revision; refused outside review with nothing sent",
	...
	{Path: []string{"cruise", "result"}, Summary: "an accepted job's result: the badge exactly as served, the provenance chain, and every limitation", Surface: "hosted",
		Args: []Arg{{Name: "job", Required: true}},
		Flags: []Flag{{Name: "download", Kind: flagBool, Summary: "also download the artifact: its sha256 and, for a verified result, the tree it holds are checked against the candidate the receipt verified before anything is written"},
			{Name: "out", Kind: flagString, Value: "FILE", Summary: "where --download writes; default JOB.tar.gz; an existing file is never written over"}},
		Effects:  "reads the job's result; with --download writes one new file after both checks pass, never over an existing file, and never extracts or runs it. accepted_without_provenance is never shown as verified",
		Examples: []string{"ks cruise result <job>", "ks cruise result <job> --download --out result.tar.gz"},
		Nothing:  "Nothing was read or written.", Run: cruiseResult},
	{Path: []string{"cruise", "apply"}, Summary: "apply an accepted job's result to this project file by file, only onto the base each file was changed from, transactionally", Surface: "hosted",
		Args: []Arg{{Name: "job", Required: true}},
		Flags: []Flag{
			{Name: "dir", Kind: flagString, Value: "DIR", Summary: "the project root to apply to (default this project's root)"},
			{Name: "confirm", Kind: flagString, Value: "DIGEST", Summary: "the changeset's digest (the 12 characters shown, or all of it); required without a terminal, never implied by --yes"},
		},
		Effects:  "downloads the result's changeset and checks its sha256 (the served header and the bytes) against the job's result before reading it; then checks every file against its recorded before-state and refuses the WHOLE apply on any local difference, so conflicting local work is kept; once confirmed, backs up, applies in a journal and verifies each file. An interrupted apply is put back with ks result apply --recover. Nothing is committed, pushed, run or deployed",
		Examples: []string{"ks cruise apply <job>", "ks cruise apply <job> --confirm 3f2a1c9b8d7e"},
		Nothing:  "Nothing was changed.", Run: cruiseApply},
	{Path: []string{"cruise", "artifact"}, Summary: "download the accepted tarball", Surface: "hosted",
		Effects:  "writes one new file locally after its sha256 matches the job record (and, where the result is served verified, after its tree matches the verified candidate); never over an existing file, never extracts or runs it",
