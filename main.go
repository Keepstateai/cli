...
var registry = []*Command{
	...
	{Path: []string{"agent", "list"}, Summary: "the agents of a session: name, id, activity, role, current task", Surface: "hosted",
		Flags:    []Flag{{Name: "session", Kind: flagString, Value: "ID", Summary: "the session the agents live in; a short id is accepted when it is unique among yours"}},
	...
	{Path: []string{"agent", "approve"}, Summary: "approve one permission request an agent is waiting on, by its id", Surface: "hosted",
		...
		Flags:    []Flag{{Name: "session", Kind: flagString, Value: "ID", Summary: "the session the request belongs to; a short id is accepted when it is unique among yours"}},
		Needs:    "agent.workspace",
	...
	{Path: []string{"agent", "deny"}, Summary: "deny one permission request an agent is waiting on, by its id", Surface: "hosted",
		Args:     []Arg{{Name: "request", Required: true}},
		Flags:    []Flag{{Name: "session", Kind: flagString, Value: "ID", Summary: "the session the request belongs to; a short id is accepted when it is unique among yours"}},
		Needs:    "agent.workspace",
	...
	{Path: []string{"agent", "pause"}, Summary: "save the session an agent lives in and park it, waiting until it is parked", Surface: "hosted",
		Args:     []Arg{{Name: "name", Required: true}},
		Flags:    []Flag{{Name: "session", Kind: flagString, Value: "ID", Summary: "the session the agent lives in; a short id is accepted when it is unique among yours"}},
		Needs:    "agent.workspace",
	...
	{Path: []string{"task", "list"}, Aliases: [][]string{{"task", "ls"}}, Summary: "the instructions of a session's agents, in the order they were committed", Surface: "hosted",
		Flags: []Flag{
			{Name: "session", Kind: flagString, Value: "ID", Summary: "the session whose instructions to read; a short id is accepted when it is unique among yours"},
	...
	{Path: []string{"task", "resume"}, Summary: "run one blocked instruction again as a new attempt; it never continues the rest of the queue instead", Surface: "hosted",
		Args: []Arg{{Name: "instruction", Required: true}},
		Flags: []Flag{
		...
		Needs:    "agent.workspace",
		Effects:  "running an instruction again is a NEW attempt under it, started from a recorded boundary, and the attempt that already ran keeps its outcome and its cost — a retry is never a rewrite of the attempt before it. This service records no attempt under an instruction and no boundary for one to resume from, so this verb REFUSES: it states which instruction is blocked, what it last reported, what nobody established, what is waiting behind it and exactly what is missing, and then it stops. Nothing is released, nothing is decided, nothing is started, nothing is metered and no boundary is ever chosen for you. Continuing the instructions held behind it is a different decision with its own command (ks agent queue resume), and this one never performs it in its place",
		Examples: []string{"ks task resume tsk_9c21 --session 3f2a1c", "ks task resume tsk_9c21 --session 3f2a1c --json"},
