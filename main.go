...
var registry = []*Command{
	...
	{Path: []string{"agent", "open"}, Summary: "the agent window: its state, then its events as they happen", Surface: "hosted",
		...
		Needs:    "agent.workspace",
		Effects:  "opens (or rejoins) a window on an agent that already exists and follows its events: it creates no agent, queues nothing, and Ctrl-C detaches the window while the agent keeps working; with --take-control the control lease moves here and the other window keeps watching",
	...
	{Path: []string{"agent", "tell"}, Summary: "queue one instruction for an agent", Surface: "hosted",
		Args:     []Arg{{Name: "name", Required: true}, {Name: "instruction", Required: true}},
		Flags:    []Flag{{Name: "session", Kind: flagString, Value: "ID", Summary: "the session the agent lives in; a short id is accepted when it is unique among yours"}},
		Needs:    "agent.workspace",
		Effects:  "adds one instruction to the agent's queue and the agent's work on it is metered like the session; the instruction is submitted once, under an id recorded locally before it is sent, so a repeat is the same submission and never a second one",
