...
var registry = []*Command{
	...
	{Path: []string{"agent", "approve"}, Summary: "approve one permission request an agent is waiting on, by its id", Surface: "hosted",
		...
		Needs:    "agent.workspace",
		Effects:  "reads the request again and approves exactly the version it read: the agent proceeds with that action, and its work is metered like the session; a request that changed, expired or was already decided is refused and nothing is approved",
	...
	{Path: []string{"agent", "deny"}, Summary: "deny one permission request an agent is waiting on, by its id", Surface: "hosted",
		...
		Flags:    []Flag{{Name: "session", Kind: flagString, Value: "ID", Summary: "the session the request belongs to; a short id is accepted when it is unique among yours"}},
		Needs:    "agent.workspace",
		Effects:  "reads the request again and denies exactly the version it read: the agent is refused that action and carries on without it; a request that changed, expired or was already decided is refused and nothing is denied",
