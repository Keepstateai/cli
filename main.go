...
var registry = []*Command{
	...
	{Path: []string{"task", "resume"}, Summary: "continue the instructions held behind one named instruction; that instruction is not restarted", Surface: "hosted",
		...
		Needs:    "agent.workspace",
		Effects:  "the same decision as ks agent queue resume, named from the other end: you give the instruction whose queue is waiting rather than the agent. It releases what is held behind that instruction and does NOT run the instruction itself again — its recorded outcome stands exactly as it is. An instruction that is not the one holding its queue is refused, and names the one that is; a queue that changed since it was displayed is displayed and refused without sending any decision",
		Examples: []string{"ks task resume tsk_9c21 --session 3f2a1c --finding \"checked by hand: the publish never happened\""},
		Nothing:  "Nothing was resumed.", Run: hosted(hostedTaskResume)},
